...
int		width = 800;
int		height = 800;
int		xM_min = -200, yM_min = -200, xM_max = 200, yM_max = 200;
int		fov = 45;
float	zNear = 0.01;
float	zFar = 10000.0;
...
void polygon(int a, int b, int c, int d, int cc)
{
	vColors[Index] = colors[cc]; vPositions[Index] = positions[a]; Index++;
	vColors[Index] = colors[cc]; vPositions[Index] = positions[b]; Index++;
	vColors[Index] = colors[cc]; vPositions[Index] = positions[c]; Index++;
	vColors[Index] = colors[cc]; vPositions[Index] = positions[a]; Index++;
	vColors[Index] = colors[cc]; vPositions[Index] = positions[c]; Index++;
	vColors[Index] = colors[cc]; vPositions[Index] = positions[d]; Index++;
}
void colorcube()
{
	polygon(1, 0, 3, 2, 0);
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("(Advanced) Rotating Cube 3D");
