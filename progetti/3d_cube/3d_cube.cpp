...
/*
* nella crazione della scena uso una unica primitiva cubo poi la riuso per 5 volte
* avremo 5 gldrawarray, ma un unico vettore dei vertici che viene passata inizialmente in gpu
* 
*/
...
//vettori che contengono info posizione camera
...
//qui dico come deve essere fatto il mio cubo
...
void init(void)
{
	...
	//glbufferdata passa informazioni per un unico cubo posizionato in origine alla gpu
	//ora passaggio da cpu a gpu sarà solo di comandi, perché dati sono già in gpu
...
/*con avanti e indietro cambio il field of view, cioè angolo di vista, che crea effetto allontanamento/avvicinamento scena
*/
void mousewheel(int button, int dir, int x, int y)
{
	if (dir > 0)
		fov += 1; //field of view, angolo di visione
	else
		fov -= 1;
	glutPostRedisplay();
}
...
void drawScene(void)
{
	...
	//la matrice la devo cambiare ogni volta che cambia qualcosa (Ad esempio fov), quindi deve essere reinviata in gpu
	...
	View = lookAt(cameraPos, cameraPos + cameraFront, cameraUp); //matrice di vista, ha tre vettori: dove si trova camera, dove sta puntando, posizione up
	...
	*/
	for (i = 0; i < n_cubi; i++)
	{
		...
		//creazione matrice di modello
		Model = mat4(1.0);
		//cubePosition trasla tutti i cubi in modo diverso
...
int main(int argc, char* argv[])
{
	...
	//da qui in poi definisco le callbacks
...
/*a partire da questo progetto, modificarlo affinché
* il nuovo programma disegni un solo cubo, cambiare colore cubo da colori ai vertici
* a colori alla faccia (no sfumature)
* permettere di scalare di un fattore di scala lungo gli assi grazie al mouse
* quindi devo definire callback che fa scaling per x/y/z
* quindi la scala deve essere interattiva
* */